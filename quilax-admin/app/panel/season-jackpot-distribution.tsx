...
interface SeasonJackpotDistribution {
  positionRules: PositionRule[];
}

export default function SeasonJackpotDistributionScreen() {
  const router = useRouter();
  const [distribution, setDistribution] = useState<SeasonJackpotDistribution>({
    positionRules: [
  ...
  const totalPercentage = distribution.positionRules.reduce((sum, rule) => sum + rule.percentage, 0);
  ...
  const handleSave = async () => {
    ...
    try {
      setSaving(true);
      const token = await AsyncStorage.getItem('authToken');
      const response = await fetch(`${API_BASE_URL}/admin/prize-config/season-jackpot-distribution`, {
        method: 'PUT',
        headers: {
  ...
  return (
    <ScrollView style={styles.container}>
      <View style={styles.header}>
        ...
        <Text style={styles.title}>Reparto de Jackpot de Temporada</Text>
      </View>

      <View style={styles.content}>
        <Text style={styles.description}>
          Configura el reparto del jackpot de temporada para TODAS las temporadas. Los cambios se aplicarán a todas las temporadas futuras.
        </Text>
        ...
        <Text style={styles.warningText}>
          ⚠️ Los cambios se aplicarán a TODAS las temporadas futuras
        </Text>
      </View>
    </ScrollView>
  );
